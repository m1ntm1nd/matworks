...
for i = 1:len
    if (abs(usersMatrixPolar(i, 2)) < pi/6)
        x = usersMatrix(i, 1);
        y = usersMatrix(i, 2);
        [dd1, dd2] = calcTDoA(basis_towers_coord_X, basis_towers_coord_Y, x, y);
        [detectedMatrix(i, 1), detectedMatrix(i, 2)] = calcPosition(dd1, dd2, basis_towers_coord_X, basis_towers_coord_Y);
        scatter(detectedMatrix(i, 1), detectedMatrix(i, 2));
    else
        theta = pi/3
        while theta < pi
            if (abs(usersMatrixPolar(i, 2)) < theta)
                if usersMatrixPolar(i, 2) > 0
                    rotate = -(theta-pi/6);
                else
                    rotate = (theta-pi/6);
                end
                [fake_x, fake_y] = ToDecartesDot(usersMatrixPolar(i, 1), usersMatrixPolar(i, 2)+rotate);
                [dd1, dd2] = calcTDoA(basis_towers_coord_X, basis_towers_coord_Y, fake_x, fake_y);
                [fake_detected_x, fake_detected_y] = calcPosition(dd1, dd2, basis_towers_coord_X, basis_towers_coord_Y);
                [fake_r, fake_phi] = ToPolarDot(fake_detected_x, fake_detected_y);
                [detectedMatrix(i, 1), detectedMatrix(i, 2)] = ToDecartesDot(fake_r, fake_phi-rotate);
                scatter(detectedMatrix(i, 1), detectedMatrix(i, 2));
            end
            theta = theta + pi/6;
        end
