...
function [towers_coord_X,towers_coord_Y] = addTowersRec2(numTiers, isd, rad)
    ...
    for c = 1:length(towers_coord_X)
        ...
        [user_X1,user_Y1] = distributeUsers(center_1_X, center_1_Y, rad, 1, 2)
        %[user_X2,user_Y2] = distributeUsers(center_2_X, center_2_Y, rad, 2, 2)    
        %[user_X3,user_Y3] = distributeUsers(center_3_X, center_3_Y, rad, 3, 2)    
...
function [gx, gy] = distributeUsers(xc,yc,rad, numPolygon, towerDist)
    N = 10000; %Number of users
    R = rad; %Radius of Hexagon
    gy=[]
    gx=[]
    v_x = (R * cos((0:5)*pi/3))+xc;
    v_y = (R * sin((0:5)*pi/3))+yc;
   ...
   elseif numPolygon == 1
    ...
    v_x = horzcat(v_x(1:t), r_x, v_x(t+1:length(v_x)))
    v_y = horzcat(v_y(1:t), r_y, v_y(t+1:length(v_y)))
